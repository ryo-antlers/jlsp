...
// ドコモアフィリエイト（バリューコマースとは別系統）。固定トラッキングリンクで、
// 特定試合への深リンクは不可。空文字なら未提携=ボタン非表示。
// ドコモスポーツくじ（WINNER/toto/BIG）。
export const WINNER_URL = 'https://tr.affiliate-sp.docomo.ne.jp/cl/d0000000359/5059/3'

// 各プログラムの pid。提携して広告コードを発行したら埋める（未提携は空文字）。
export const VC_PID = {
  jalan: '892646740', // じゃらんnet（宿・ホテル）
  jalanRentacar: '892646755', // じゃらんレンタカー
  yahooTravel: '', // Yahoo!トラベル（保留：宿はじゃらんと重複・高速バスは別ASP検討）
}
...
/**
 * クラブ本拠地の都道府県でレンタカー検索（じゃらんレンタカー）への成果リンク。
 * 県コードはホテルエリアURL（/{県コード}/CTY_…）から再利用。
 * エリア未登録なら じゃらんレンタカー トップに fallback。未提携なら null。
 */
export function jalanRentacarLink(clubId) {
  const prefCode = JALAN_AREA[clubId]?.url.match(/jalan\.net\/(\d+)\//)?.[1]
  const target = prefCode
    ? `https://www.jalan.net/rentacar/search/${prefCode}/`
    : 'https://www.jalan.net/rentacar/'
  return vcLink('jalanRentacar', target)
}

...
/**
 * 主なOB選手の「このクラブでの出場数・ゴール数」を一括取得。
 *
 * - 名前のスペース有無差を吸収 (DB: '内田 篤人' vs NOTABLE_ALUMNI: '内田篤人')
 * - 同名で canonical_id が複数あるケースは出場数の多い方を採用
 * - 集計対象: league_apps/goals + cup_apps/goals + cs_league_apps/goals
 *
...
export async function loadAlumniStats(teamId, names) {
  ...
  const rows = await sql`
    SELECT
      ...
      SUM(
        COALESCE(pcs.league_apps, 0) +
        COALESCE(pcs.cup_apps, 0) +
        COALESCE(pcs.cs_league_apps, 0)
      )::int AS apps,
      SUM(
        COALESCE(pcs.league_goals, 0) +
        COALESCE(pcs.cup_goals, 0) +
        COALESCE(pcs.cs_league_goals, 0)
      )::int AS goals

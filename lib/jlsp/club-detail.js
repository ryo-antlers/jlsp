...
export async function loadClubDetail(teamId) {
  ...
  const [standingsRows, recentRows, upcomingRows, playerRows, stadiumRows] = await Promise.all([
    ...
    sql`
      ...
      LIMIT 3
    `.catch(() => []),
    // 得点ランキング: fixture_player_stats を集計、ゴール数上位 5 名
    sql`
      ...
      HAVING COALESCE(SUM(fps.goals), 0) > 0
      ORDER BY goals DESC, assists DESC, minutes DESC NULLS LAST
      LIMIT 5
    `.catch(() => []),
    sql`

...
if (QUESTIONS.length === 48) ok(`QUESTIONS count = 48 (warm-up 16 + soccer 32)`)
else fail(`QUESTIONS count expected 48, got ${QUESTIONS.length}`)

const warmupCount = QUESTIONS.filter((q) => q.id.startsWith('w')).length
const soccerCount = QUESTIONS.filter((q) => q.id.startsWith('q')).length
if (warmupCount === 16 && soccerCount === 32) ok(`warm-up 16 / soccer 32`)
else fail(`warm-up/soccer split: w=${warmupCount} q=${soccerCount}`)
...
// 4軸ごとに +/- それぞれ 6 問あること (warm-up 2 + soccer 4)
...
if (decoded && decoded.length === 48 && decoded.every((a, i) => a.step === rambo[i].step)) ok(`encode/decode round-trip OK (encoded len=${encoded.length})`)
...
// パターン5: scoreAnswers の最大値 = MAX_PER_AXIS の範囲を超えない (12問 × 3 = 36)
...
if (process.env.DATABASE_URL) {
  ...
  // DB 適用後の clubs と JSON 経由の CLUBS は同じであるべき (seed が JSON 由来なので)
  ...
  for (const c of CLUBS) {
    ...
    for (const axisId of AXIS_IDS) {
      if (c.vector[axisId] !== d.vector[axisId]) mismatch++
    }
  }
  if (mismatch === 0) ok(`DB 経路と JSON 経路の vector が完全一致 (40 クラブ)`)

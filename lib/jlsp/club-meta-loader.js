...
export async function loadMergedClubMeta() {
  const rows = await sql`
    SELECT club_id, description_long, sightseeing, notable_alumni, mascots, stadium, current_players
  ...
  for (const [id, base] of Object.entries(CLUB_META)) {
    ...
    merged[id] = {
      ...
      // 現在所属の有名選手 (名前のみ・admin手入力)。
      currentPlayers: Array.isArray(o.current_players) ? o.current_players : [],
...
export async function saveClubMetaOverride(clubId, fields) {
  ...
  const currentJson = current_players == null ? null : JSON.stringify(current_players)
  await sql`
    INSERT INTO club_meta_overrides
      (club_id, description_long, mascots, stadium, sightseeing, notable_alumni, current_players, updated_at)
    VALUES
      (${clubId}, ${description_long}, ${mascotsJson}::jsonb, ${stadium},
       ${sightseeingJson}::jsonb, ${alumniJson}::jsonb, ${currentJson}::jsonb, NOW())
...
export async function loadAllClubMetaOverrides() {
  const rows = await sql`
    SELECT club_id, description_long, sightseeing, notable_alumni, mascots, stadium, current_players, updated_at
    FROM club_meta_overrides
  `.catch(() => [])
  const byId = {}

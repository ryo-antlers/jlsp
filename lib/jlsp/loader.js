...
export async function loadJlspState() {
  ...
  // vector + 色 + 質問単位 override を一括適用
  // diagnose.js の clubExpectedAnswer は club.overrides[q.id] を参照するので
  // ここで questionOverrides を inline 化する
  const allClubs = applyVectorOverrides(vectorOverrides).map((c) => {
    const color = colorByClubId[c.id]
    const overrides = questionOverrides[c.id]
    return {
      ...c,
      ...(color ? { color } : {}),
